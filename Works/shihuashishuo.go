...
func Shihuashishuo_DoWork(rootUrl string,crawlerChan chan string,collectionName string){

	collector := colly.NewCollector()

	//爬取下一页链接
	collector.OnHTML("div#pages", func(e *colly.HTMLElement) {
		GetNextPage_Shihuashishuo(collector,e)
	})

	//爬取每页的公告url
	collector.OnHTML("ul.photo-list.picbig>li", func(e *colly.HTMLElement) {
		//log.Printf("爬取每页的公告url")
		GetDetailUrl_Shihuashishuo(e,collector, collectionName)
	})

	//获取详细信息
	collector.OnHTML("div#Article", func(e *colly.HTMLElement){
		//log.Printf("shihuashishuo -- 获取详细信息")
		GetDetailInfo_Shihuashishuo(e,collectionName)
	})

	collector.Visit(rootUrl)
	crawlerChan <- "石话实说爬取结束"

}


//获取下一页链接
func GetNextPage_Shihuashishuo(collector *colly.Collector, element *colly.HTMLElement) {
	...
	url := "http://www.sqxw.gov.cn/html/shipin/shihuashishuo/"+strconv.Itoa(nextIndex)+".html"
	collector.Visit(url)
}

//爬取每页的公告url
func GetDetailUrl_Shihuashishuo( e *colly.HTMLElement, collector *colly.Collector,collectionName string) {
...
//获取详细信息
func GetDetailInfo_Shihuashishuo(e *colly.HTMLElement, collectionName string) {
	...
	if title == "" || timer == ""||len(contentList) <= 0 || vedioUrl == "" || len(vedioUrl) <= 0 ||
		len(title) <= 0 || len(timer) <= 0 || len(vedioUrl) <= 0 {
