...
func Taitea_DoWork(rootUrl string,crawlerChan chan string,collectionName string){
	...
	collector.OnHTML("div#pages", func(e *colly.HTMLElement) {
		GetNextPage_Taitea(collector,e)
	})

	//爬取每页的公告url
	collector.OnHTML("ul.list.lh24.f14>li", func(e *colly.HTMLElement) {
		//log.Printf("爬取每页的公告url")
		GetDetailUrl_Taitea(e,collector, collectionName)
	})

	//获取详细信息
	collector.OnHTML("div#Article", func(e *colly.HTMLElement){
		//log.Printf("获取详细信息")
		GetDetailInfo_Taitea(e,collectionName)
	})

	collector.Visit(rootUrl)
	crawlerChan <- "苔茶文化爬取结束"

}


//获取下一页链接
func GetNextPage_Taitea(collector *colly.Collector, element *colly.HTMLElement) {
	...
	url := "http://www.sqxw.gov.cn/html/wenhua/taichawenhua/"+strconv.Itoa(nextIndex)+".html"
	collector.Visit(url)
}

//爬取每页的公告url
func GetDetailUrl_Taitea(element *colly.HTMLElement, collector *colly.Collector,collectionName string) {
	url := element.ChildAttr("a","href")
	if url == "" || len(url) <= 0 {
		return
	}

	if err := utils.InsertDetailUrl(collectionName, url,models.Taitea{}); err != nil {
		log.Printf("%s",err)
	}
...
//获取详细信息
func GetDetailInfo_Taitea(e *colly.HTMLElement, collectionName string) {
