...
func Yelang_DoWork(rootUrl string,crawlerChan chan string,collectionName string){
	...
	collector.OnHTML("ul.list.lh24.f14>li", func(e *colly.HTMLElement) {
		...
		GetDetailUrl_Yelang(e,collector, collectionName)
	})

	//获取详细信息
	collector.OnHTML("div#Article", func(e *colly.HTMLElement){
		//log.Printf("获取详细信息")
		GetDetailInfo_Yelang(e,collectionName)
	})

	collector.Visit(rootUrl)
	crawlerChan <- "夜郎文化爬取结束"

}


//获取下一页链接
func GetNextPage_Yelang(collector *colly.Collector, element *colly.HTMLElement) {
	...
	url := "http://www.sqxw.gov.cn/html/wenhua/hongsewenhua/"+strconv.Itoa(nextIndex)+".html"
	collector.Visit(url)
}

//爬取每页的公告url
func GetDetailUrl_Yelang(element *colly.HTMLElement, collector *colly.Collector,collectionName string) {
	url := element.ChildAttr("a","href")
	if url == "" || len(url) <= 0 {
		return
	}

	if err := utils.InsertDetailUrl(collectionName, url,models.RedCulture{}); err != nil {
		log.Printf("%s",err)
	}
...
//获取详细信息
func GetDetailInfo_Yelang(e *colly.HTMLElement, collectionName string) {
