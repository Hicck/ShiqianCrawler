...
func Minsu_DoWork(rootUrl string,crawlerChan chan string,collectionName string){

	collector := colly.NewCollector()

	//爬取下一页链接
	collector.OnHTML("div#pages", func(e *colly.HTMLElement) {
		GetNextPage_Minsu(collector,e)
	})

	//爬取每页的公告url
	collector.OnHTML("ul.list.lh24.f14>li", func(e *colly.HTMLElement) {
		//log.Printf("爬取每页的公告url")
		GetDetailUrl_Minsu(e,collector, collectionName)
	})

	//获取详细信息
	collector.OnHTML("div#Article", func(e *colly.HTMLElement){
		//log.Printf("获取详细信息")
		GetDetailInfo_Minsu(e,collectionName)
	})

	collector.Visit(rootUrl)
	crawlerChan <- "民俗文化爬取结束"

}


//获取下一页链接
func GetNextPage_Minsu(collector *colly.Collector, element *colly.HTMLElement) {
...
//爬取每页的公告url
func GetDetailUrl_Minsu(element *colly.HTMLElement, collector *colly.Collector,collectionName string) {
...
//获取详细信息
func GetDetailInfo_Minsu(e *colly.HTMLElement, collectionName string) {
