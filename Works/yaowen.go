...
func Yaowen_DoWork(rootUrl string,crawlerChan chan string,collectionName string){

	collector := colly.NewCollector()

	//爬取下一页链接
	collector.OnHTML("div#pages", func(e *colly.HTMLElement) {
		GetNextPage_Yaowen(collector,e)
	})

	//爬取每页的公告url
	collector.OnHTML("div.col-left>ul.list.lh24.f14>li", func(e *colly.HTMLElement) {
		//log.Printf("爬取每页的公告url")
		GetDetailUrl_Yaowen(e,collector, collectionName)
	})

	//获取详细信息
	collector.OnHTML("div#Article", func(e *colly.HTMLElement){
		//log.Printf("获取详细信息")
		GetDetailInfo_Yaowen(e,collectionName)
	})

	collector.Visit(rootUrl)
	crawlerChan <- "石阡要闻爬取结束"

}


//获取下一页链接
func GetNextPage_Yaowen(collector *colly.Collector, element *colly.HTMLElement) {
	...
	url := "http://www.sqxw.gov.cn/html/xinwen/shiqianyaowen/"+strconv.Itoa(nextIndex)+".html"
	collector.Visit(url)
}

//爬取每页的公告url
func GetDetailUrl_Yaowen(element *colly.HTMLElement, collector *colly.Collector,collectionName string) {
	url := element.ChildAttr("a","href")
	if url == "" || len(url) <= 0 {
		return
	}

	if err := utils.InsertDetailUrl(collectionName, url,models.Yaowen{}); err != nil {
		log.Printf("%s",err)
	}
...
//获取详细信息
func GetDetailInfo_Yaowen(e *colly.HTMLElement, collectionName string) {
