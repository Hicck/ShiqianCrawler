...
func Bumendongtai_DoWork(rootUrl string,crawlerChan chan string,collectionName string){

	collector := colly.NewCollector()

	//爬取下一页链接
	collector.OnHTML("div#pages", func(e *colly.HTMLElement) {
		GetNextPage_Bumendongtai(collector,e)
	})

	//爬取每页的公告url
	collector.OnHTML("div.col-left>ul.list.lh24.f14>li", func(e *colly.HTMLElement) {
		//log.Printf("爬取每页的公告url")
		GetDetailUrl_Bumendongtai(e,collector, collectionName)
	})

	//获取详细信息
	collector.OnHTML("div#Article", func(e *colly.HTMLElement){
		//log.Printf("获取详细信息")
		GetDetailInfo_Bumendongtai(e,collectionName)
	})

	collector.Visit(rootUrl)
	crawlerChan <- "部门动态爬取结束"

}


//获取下一页链接
func GetNextPage_Bumendongtai(collector *colly.Collector, element *colly.HTMLElement) {
	...
	url := "http://www.sqxw.gov.cn/html/bumen/"+strconv.Itoa(nextIndex)+".html"
	collector.Visit(url)
}

//爬取每页的公告url
func GetDetailUrl_Bumendongtai(element *colly.HTMLElement, collector *colly.Collector,collectionName string) {
	url := ""
	bumen := ""
	element.ForEach("a", func(i int, element *colly.HTMLElement) {
		...
		if i == 0 {
			bumen = element.Text
			bumen = convert.ConvertToString(bumen, "gbk", "utf-8")
		}
		if i == 1{
			url = element.Attr("href")
		}
	})
	if len(url) <= 0 || url == "" || bumen == ""{
		return
	}
	//如果数据库中已经有该数据
	if utils.CheckWebsite(collectionName,url) == false{
		info := models.Bumendongtai{
			Website:url,
			Bumen:bumen,
		}
...
//获取详细信息
func GetDetailInfo_Bumendongtai(e *colly.HTMLElement, collectionName string) {
